...
def donchian_breakout_setup(df: pd.DataFrame, stop_atr_mult: float = 2.0, target_atr_mult: float = 3.0, lookback: int = 20) -> Optional[Dict]:
    if len(df) < max(lookback, 50): return None
    atr = compute_atr(df).iloc[-1]; entry = float(df["close"].iloc[-1])
    breakout = float(rolling_high(df, lookback).iloc[-2])
    if entry <= breakout: return None
...
def trend_rsi_pullback_setup(df: pd.DataFrame, stop_atr_mult: float = 2.0, target_high_lookback: int = 20) -> Optional[Dict]:
    ...
    cl = df["close"]; sma50 = cl.rolling(50).mean().iloc[-1]; sma200 = cl.rolling(200).mean()
    if not (cl.iloc[-1] > sma50 and sma200.iloc[-1] > sma200.iloc[-5]): return None
    r = rsi(cl).iloc[-1]; if r >= 35: return None
    atr = compute_atr(df).iloc[-1]; entry = float(cl.iloc[-1]); stop = entry - stop_atr_mult * atr
    target = float(df["high"].rolling(target_high_lookback).max().iloc[-2]); 
    if target <= entry: target = entry + 2.0 * atr
    rr = rr_ratio(entry, stop, target, "long")
...
def donchian_breakdown_setup(df: pd.DataFrame, stop_atr_mult: float = 2.0, target_atr_mult: float = 3.0, lookback: int = 20) -> Optional[Dict]:
    ...
    if entry >= breakdown: return None
    stop = entry + stop_atr_mult * atr; target = entry - target_atr_mult * atr
...
def trend_rsi_rip_setup(df: pd.DataFrame, stop_atr_mult: float = 2.0, target_low_lookback: int = 20) -> Optional[Dict]:
    ...
    cl = df["close"]; sma50 = cl.rolling(50).mean().iloc[-1]; sma200 = cl.rolling(200).mean()
    if not (cl.iloc[-1] < sma50 and sma200.iloc[-1] < sma200.iloc[-5]): return None
    rrsi = rsi(cl).iloc[-1]; if rrsi <= 65: return None
    atr = compute_atr(df).iloc[-1]; entry = float(cl.iloc[-1]); stop = entry + stop_atr_mult * atr
