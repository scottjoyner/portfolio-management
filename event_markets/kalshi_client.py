...
class KalshiClient:
    ...
    def _sign_request(self, timestamp_ms: str, method: str, path: str) -> str:
        # Validate the timestamp is within a sane range so a wildly skewed
        # clock (or bad env override) surfaces as a clear error rather than a
        # confusing 401 signature-rejected loop.
        try:
            ts_val = int(timestamp_ms)
        except (ValueError, TypeError):
            raise RuntimeError("Kalshi signature timestamp must be an integer ms string")
        if ts_val <= 0:
            raise RuntimeError("Kalshi signature timestamp is non-positive")
    ...
    def _auth_header(self, method: str = "GET", path: str = "") -> Dict[str, str]:
        if self._use_api_key_auth():
            ts = str(int(self._request_time_ms() * 1000))
            return {
    ...
    # Offset (seconds) added to the signature timestamp to tolerate clock skew
    # between this host and Kalshi's servers. Kalshi rejects signatures whose
    # timestamp deviates by more than a small window; if this host clock runs
    # behind, a negative offset keeps the request inside the acceptance window.
    # Set via env KALSHI_CLOCK_SKEW_SECONDS (may be negative).
    def _clock_skew_seconds(self) -> float:
        raw = os.environ.get("KALSHI_CLOCK_SKEW_SECONDS", "")
        if raw:
            try:
                return float(raw)
            except ValueError:
    ...
    def _request_time_ms(self) -> float:
    ...
    def _get(self, path: str, params: Optional[Dict[str, Any]] = None, _retries: int = 3) -> Any:
        ...
        try:
            ...
            # Token-expiry / auth rejection. The legacy email/password token
            # can expire; re-login once and retry. For API-key auth the
            # signature is timestamped and effectively expires within Kalshi's
            # skew window, so a fresh timestamp+sig is generated by re-building
            # the auth header on retry (no stored credential to refresh).
            if e.code == 401 and self._token:
                self._login()
                return self._get(path, params, _retries)
            if e.code == 401 and self._use_api_key_auth() and _retries > 0:
                time.sleep(0.1)
    ...
    def _write(self, method: str, path: str, body: Optional[Dict[str, Any]] = None, _retries: int = 3) -> Any:
        ...
        Retries on HTTP 429 (rate limit, with backoff) and 401 (stale
        timestamp signature), since the timestamped signature can fall outside
        Kalshi's acceptance window if the host clock drifts.
        """
        ...
        try:
            ...
            try:
                ...
                pass
            if e.code in (401, 429) and _retries > 0:
                wait = float(e.headers.get("Retry-After", 0) or 0) or 0.1
                time.sleep(min(wait, 10.0))
                return self._write(method, path, body, _retries - 1)
