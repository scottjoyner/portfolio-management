...
| E13 | **TODO (blocked on data):** once the Coinbase 365d backfill populates enough 1h/1d candles across the top-50 pairs, run a full cross-symbol backtest (`backtest_from_cache.py --granularity 3600` over all harvested symbols) and persist verdicts into `optimizer_state.db` via `--bt_cache_db`, so live confidence scoring reflects measured edge. | Data | M | TODO |
