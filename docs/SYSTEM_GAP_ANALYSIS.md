...
| E12 | Backtest loader reads harvested `feed_cache` parquet and runs all 74 Rust strategies via `strategy_engine.backtest_strategy`, producing a ranked pass/fail report (`scripts/backtest_from_cache.py`). Default = Rust strategies only (fast); `--include-external` adds the 9 Python-path strategies; `--max-bars` caps the trailing window. **Feedback loop closed**: `--bt-cache-db optimizer_state.db` writes verdicts into the optimizer's `bt_cache` SQLite (keyed `strategy/currency`), which `ConfidenceMatrix` consumes as per-strategy weights (`confidence_weight_from_bt_py`). KNOWN: Rust `impulse_exh` + `liq_vac` panic on backtest (strategies.rs:1078/1216) — caught and skipped by the loader; separate Rust bug to fix. | Data | M | DONE |
| E13 | **One-shot cross-symbol backtest job** (`scripts/backtest_all_harvested.py`) auto-discovers harvested symbols @1h, backtests via the E12 loader, and persists verdicts into the optimizer `bt_cache` (`--bt-cache-db`). Full top-50 cross-symbol run remains **blocked on backfill** completing 1h/1d legs; partial runs (5 symbols) validated. | Data | M | DONE |
| E14 | **Iterative backtesting framework** — reproducible, comparable experiments so every new config/strategy change is evaluated against history with a pass/fail verdict and a diff. `scripts/backtest_framework/`: `experiment.py` (Experiment spec: strategies/universe/asset-classes/granularity/window/thresholds + `walk_forward_folds`, decoupled from live yaml modes), `run_experiment.py` (executor reusing `strategy_engine.backtest_strategy` per (strategy,symbol) for Rust-panic resilience; writes `experiments/<name>/scorecard.json` + appends `ledger.jsonl`; optional `--bt-cache-db` to feed `ConfidenceMatrix`), `compare_experiments.py` (scorecard diff + **regression gate**: fails if aggregate mean-sharpe drops, a passing strategy now fails, or pass-rate drops > 5%), `walk_forward.py` (**v2**: OOS folds via `make_folds`/`walk_forward`/`aggregate_walk_forward` — reports OOS mean sharpe + `stable` strategies that pass both IS and OOS), `promote.py` (**v2**: turns a passing scorecard into a `trading_system/configs/*.yaml` mode listing validated `enabled_strategies` + a companion `.promoted.json`; always PAPER mode, never auto-live). Unit tests in `tests/coverage/backtest_framework/` (12 tests). Reuses the same Rust engine as the paper trader → results are directly comparable to live behavior. **Validated end-to-end** on 5 harvested SSD symbols @1h (216 strategy verdicts; honest finding: 0 strategies stable out-of-sample on this window). | Backtest | M | DONE (v1+v2) |
...
- [x] `scripts/backtest_from_cache.py` added (E12): reads harvested `feed_cache` parquet, runs all 74 Rust strategies via `strategy_engine.backtest_strategy`, emits a ranked pass/fail report. `--bt-cache-db` closes the loop by writing verdicts into the optimizer `bt_cache` SQLite (keyed `strategy/currency`), consumed by `ConfidenceMatrix` as per-strategy weights.

| E15 | **Ensemble / independence-group consensus experiment** — addresses the E14 finding that *no single strategy is OOS-stable* (stable_rate 0.0% on the harvested window). The live trader never bets on one strategy; it bets when multiple **independent** strategy groups agree (`confidence_matrix.ConfidenceMatrix` boosts by group agreement). `scripts/backtest_framework/ensemble.py` computes, per symbol, how many independence groups have >=1 passing strategy and a group-weighted `ensemble_sharpe`; `consensus` = True iff `>= min_groups` (default 2) independent groups agree. Wired into `run_experiment --ensemble` (adds `ensemble` block to the scorecard: `consensus_coverage`, `n_consensus`, `mean_ensemble_sharpe`, `best_symbol`, `consensus_symbols`). Reuses the same per-strategy verdicts (free after the single-strategy run). Unit-tested (14 tests total in the framework suite; `ensemble.py` at 100% coverage). **Honest result:** on the current harvested window 0 strategies pass at all, so ensemble coverage is also 0% — but the logic correctly detects multi-group consensus (verified on synthetic passing verdicts) and will surface the tradeable universe once backfill yields a regime where *some* strategies pass. | Backtest | M | DONE |

| E16 | **Regime-conditioned ensemble** — split each symbol's window by detected regime (`rust_core.detect_regime_py`, heuristic fallback) and compute `ensemble.ensemble_consensus` per regime slice. `scripts/backtest_framework/regime_experiment.py` (`detect_regime_slices`, `regime_ensemble`) + `--regime`/`--regime-min-bars` wired into `run_experiment`. Scorecard gets a `regime` block (per-regime consensus_coverage, n_consensus, mean_ensemble_sharpe, best_regime). **Finding:** on the harvested 1h window only the `ranging` regime appears, so regime-splitting alone does not yet surface edge (no strategies pass) — but the plumbing is ready to isolate trend/downtrend segments once longer windows are harvested. | Backtest | M | DONE |
| E17 | **Multi-timeframe confirmation experiment** — a strategy "passes" only if it passes on BOTH primary granularity and a confirmation granularity (e.g. 1h + 1d). `scripts/backtest_framework/multitimeframe.py` (`mtf_backtest`, `mtf_summary`, Rust-panic-safe) + `--confirm-granularity` in `run_experiment`. Scorecard `multitimeframe` block reports mtf_pass_rate vs single_pass_rate and `lift`. **Finding:** lift=-0.3% on harvested data — confirmation is a strictly tighter filter (as expected); it will sharpen the tradeable set once strategies pass at all. | Backtest | M | DONE |
| E18 | **One-command suite + regression CI gate** — `scripts/backtest_framework/run_suite.py` runs a built-in battery (baseline_1h, baseline_1d, walkforward_1h, ensemble_1h), persists scorecards, compares each candidate to a baseline via `compare_experiments.compare`, and exits non-zero on any regression (the CI hook). `report_ledger.py` prints the historical `ledger.jsonl` for trend comparison. `scripts/experiments/run_all.sh` wraps it (sets `NAS_FEED_ROOT` to the SSD if present). This operationalizes "consistently test new configs/strategies" into a single reproducible command. | Backtest | M | DONE |
| E19 | **Rust panic fix (unblocks 2 strategies)** — `impulse_exh` + `liq_vac` panicked at `strategies.rs:1078/1216` (`index out of bounds: len 0 but 30`) because `evaluate()` passed an empty `opens` slice that those strategies index at `n-1`. Fixed in `rust_core/src/strategies.rs`: synthesize `opens` from `closes` (mirroring `evaluate_all`), plus a `.min(_opens.len())` guard returning HOLD on short data. Compiled via `maturin develop --release` (rustc 1.96); synced the rebuilt `.so` into the repo. Both strategies now return graceful verdicts (no panic) on any window, so they are no longer silently skipped in every experiment — 2 more real strategies available to the framework. | Backtest | M | DONE |
| E20 | **Paper-trader outcome replay / audit** — closes the loop between live paper trading and backtesting. `scripts/backtest_framework/replay_trades.py` loads `trade_events/<PRODUCT>.jsonl` (schema verified against `run_trader_v4.py:2192/2596/2819`: `kind`∈{entry,exit}, `product_id`, `ts`, `price`, entry `side`/`strategy`/`notional`, exit `pnl`/`reason`), pairs entries→exits, scores live win_rate/pnl, and `compare_to_backtest` flags divergence vs the backtest scorecard's per-strategy win_rate. **Finding:** no real `trade_events/*.jsonl` exist yet under the feed cache, so replay is wired and unit-tested but awaiting live trade data. Once the paper trader harvests events, this directly answers "did live trades match the backtested edge?". | Backtest | M | DONE |
