...
class FakePgClientP1 {
  ...
  async query(sql, params = []) {
    ...
    if (sql.startsWith('SELECT version FROM schema_migrations')) {
      return { rows: this.includeP1Migration ? [{ version: '001_operator_state' }, { version: '002_operator_product_layer' }] : [{ version: '001_operator_state' }] };
    }
    ...
    if (sql.includes("WHERE key IN ('accounts','instruments','strategy_templates','paper_executions')")) return { rows: [...this.flags.entries()].map(([key, value]) => ({ key, value_json: value })) };
