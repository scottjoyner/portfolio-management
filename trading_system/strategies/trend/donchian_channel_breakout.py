"""
DonchianChannelBreakoutStrategy - Trend Following

Purpose: Buy when price breaks above N-period high (resistance breakout), sell on opposite.

Regime Suitability:
  ✅ Strong trending markets with clear breakouts
...
Expected Performance:
  ...
  • Maximum historical drawdown: 20-26%
"""
...
class DonchianChannelBreakoutConfig:
    """Configuration for Donchian Channel Breakout Strategy."""
    period: int = 20      # Period for high/low calculation
    breakout_threshold: float = 0.05  # Minimum channel width for breakout (5%)
    
    # Trend filter parameters
    min_trend_strength: float = 0.3
    max_drawdown_bps: float = 260.0  # Maximum drawdown before exit (2.6%)
    ...
    exit_threshold: float = -0.03  # Maximum channel width for short (-3%)
...
class DonchianChannelBreakoutStrategy:
    ...
    Generates buy signals when price breaks above N-period high (resistance breakout).
    Generates sell signals on opposite conditions.
    """
    
    def __init__(self, config: Optional[DonchianChannelBreakoutConfig] = None):
        """
        ...
        """
        self.config = config or DonchianChannelBreakoutConfig()
    ...
    def on_bar(self, market_state: dict) -> Optional[dict]:
        ...
        channel_width = (upper_channel - lower_channel) / lower_channel if lower_channel != 0 else 0
        
        # Check if we're in a valid trending regime
        if abs(channel_width) < self.config.min_trend_strength:
            return None
        ...
        if is_long_position:
            ...
            # Exit on trend reversal (price breaks below lower channel)
            elif close_price < lower_channel and channel_width < self.config.exit_threshold:
                return {
                    'action': 'close',
                    'quantity': -self.config.risk_per_trade / close_price,
                    'stop_loss': trailing_stop,
            ...
            # Exit on trend reversal (price breaks above upper channel)
            elif close_price > upper_channel and channel_width > self.config.entry_threshold:
                return {
                    'action': 'close',
                    'quantity': -self.config.risk_per_trade / close_price,
                    'stop_loss': trailing_stop,
    ...
    def metadata(self) -> dict:
        ...
        return {
            ...
            'purpose': 'Buy on resistance breakout (price above N-period high)',
            'regime_suitability': ['Strong trending markets with clear breakouts'],
            'supported_products': ['BTC-USD', 'ETH-USD', 'SOL-USD'],
            'risk_tier': 'TIER_2_MODERATE_RISK',
            'required_data': ['close', 'upper_channel', 'lower_channel'],
            'required_indicators': ['Donchian Channel (period)'],
