"""
ParabolicSARTrendFollowingStrategy - Trend Following

Purpose: Uses Parabolic SAR for dynamic stop placement and trend following signals.

Regime Suitability:
  ✅ Strong trending markets with clear direction
  ❌ Ranging markets with frequent PSAR reversals

Failure Modes:
  • Whipsaws during sideways choppy markets
  • False signals at key support/resistance levels
  • Late entries due to PSAR lag
...
class ParabolicSARTrendFollowingConfig:
    """Configuration for Parabolic SAR Trend Following Strategy."""
    period: int = 20      # Period for initial PSAR calculation
    ...
    entry_threshold: float = 0.04  # Minimum PSAR distance for entry (4%)
    exit_threshold: float = 0.40  # Maximum PSAR distance for long exit (40%)
    ...
    cooldown_bars: int = 4
    warmup_bars: int = 60  # Bars needed for PSAR calculation
...
class ParabolicSARTrendFollowingStrategy:
    ...
    Generates buy signals when PSAR is below price and trending upward.
    Generates sell signals on opposite conditions.
    """
    
    def __init__(self, config: Optional[ParabolicSARTrendFollowingConfig] = None):
        """
        ...
        """
        self.config = config or ParabolicSARTrendFollowingConfig()
    ...
    def on_bar(self, market_state: dict) -> Optional[dict]:
        ...
        # Get PSAR values (assumed to be pre-calculated in market_state)
        psar_value = float(market_state.get('psar', 0))
        psar_trend = float(market_state.get('psar_trend', 0))  # 1 for up, -1 for down
        
        # Calculate PSAR distance from price
        psar_distance = abs(close_price - psar_value) / close_price if close_price != 0 else 0
        
        # Check if we're in a valid trending regime
        if psar_distance < self.config.min_trend_strength:
            return None
        ...
        if is_long_position:
            ...
            # Exit on trend reversal (PSAR crosses above price)
            elif psar_trend == 1 and psar_distance < self.config.exit_threshold:
                return {
                    'action': 'close',
                    'quantity': -self.config.risk_per_trade / close_price,
                    'stop_loss': trailing_stop,
            ...
            # Exit on trend reversal (PSAR crosses below price)
            elif psar_trend == -1 and psar_distance > self.config.entry_threshold:
                return {
                    'action': 'close',
                    'quantity': -self.config.risk_per_trade / close_price,
                    'stop_loss': trailing_stop,
        ...
        if psar_trend == 1 and psar_distance > self.config.entry_threshold:
            ...
            return {
                'action': 'open',
                'quantity': self.config.risk_per_trade / close_price,
                'stop_loss': psar_value * (1 - self.config.stop_loss_bps / 10000),
                'take_profit': psar_value * (1 + self.config.take_profit_bps / 10000),
                'reason': 'psar_trend_up'
            }
        elif psar_trend == -1 and psar_distance > self.config.entry_threshold:
    ...
    def metadata(self) -> dict:
        ...
        """
        return {
            'strategy_id': 'parabolic_sar_trend_following',
            'name': 'Parabolic SAR Trend Following',
            'family': 'Trend Following',
            'purpose': 'Dynamic stop placement using Parabolic SAR for trend following signals',
            'regime_suitability': ['Strong trending markets with clear direction'],
            'supported_products': ['BTC-USD', 'ETH-USD', 'SOL-USD'],
            'risk_tier': 'TIER_2_MODERATE_RISK',
            'required_data': ['close', 'psar', 'psar_trend'],
            'required_indicators': ['Parabolic SAR (period, af_start, af_max)'],
            ...
            'explainability_output': 'PSAR position relative to price and trend direction',
