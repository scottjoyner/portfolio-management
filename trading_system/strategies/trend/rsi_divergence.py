"""
RSIDivergenceStrategy - Trend Following

Purpose: Identifies bullish/bearish divergences between price and RSI for reversal signals.

Regime Suitability:
  ✅ Strong trending markets with clear overbought/oversold conditions
  ❌ Choppy/ranging markets with frequent false divergences

Failure Modes:
  • False divergences in choppy markets
  • Late entries due to divergence confirmation lag
...
class RSIDivergenceConfig:
    ...
    divergence_strength: float = 0.05  # Minimum price range for divergence (5%)
...
class RSIDivergenceStrategy:
    ...
    Generates buy signals when price makes lower lows but RSI makes higher lows (bullish divergence).
    Generates sell signals on opposite conditions (bearish divergence).
    """
    
    def __init__(self, config: Optional[RSIDivergenceConfig] = None):
        """
    ...
    def on_bar(self, market_state: dict) -> Optional[dict]:
        ...
        if rsi_divergence > self.config.entry_threshold and rsi_value < self.config.oversold_threshold:
            ...
            return {
                ...
                'stop_loss': close_price * (1 + self.config.stop_loss_bps / 10000),
                'take_profit': close_price * (1 - self.config.take_profit_bps / 10000),
                'reason': 'rsi_overbought_divergence'
            }
    ...
    def metadata(self) -> dict:
        ...
        return {
            ...
            'purpose': 'Bullish/bearish divergences between price and RSI for reversal signals',
            ...
            'failure_modes': ['False divergences in choppy markets'],
            'disable_criteria': ['Weak trend strength', 'Maximum drawdown reached'],
            'cooldown_logic': 'fixed_bar_count',
            'explainability_output': 'RSI divergence from price trend and overbought/oversold status',
