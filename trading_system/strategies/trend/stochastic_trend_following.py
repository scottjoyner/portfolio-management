"""
StochasticTrendFollowingStrategy - Trend Following

Purpose: Uses Stochastic Oscillator for trend identification and momentum signals.
...
class StochasticTrendFollowingConfig:
    """Configuration for Stochastic Trend Following Strategy."""
    ...
    exit_threshold: float = -0.02  # Maximum oscillator divergence for short (-2%)
...
class StochasticTrendFollowingStrategy:
    """
    ...
    def __init__(self, config: Optional[StochasticTrendFollowingConfig] = None):
        """
        ...
        """
        self.config = config or StochasticTrendFollowingConfig()
    ...
    def on_bar(self, market_state: dict) -> Optional[dict]:
        ...
        if oscillator_divergence > self.config.entry_threshold and k_value < self.config.oversold_threshold:
            return {
                ...
                'reason': 'stochastic_oversold_trend'
            }
            ...
            return {
                ...
                'stop_loss': close_price * (1 + self.config.stop_loss_bps / 10000),
                'take_profit': close_price * (1 - self.config.take_profit_bps / 10000),
                'reason': 'stochastic_overbought_trend'
            }
    ...
    def metadata(self) -> dict:
        ...
        """
        return {
            'strategy_id': 'stochastic_trend_following',
            'name': 'Stochastic Trend Following',
            'family': 'Trend Following',
            'purpose': 'Oscillator position relative to overbought/oversold levels and trend direction',
            ...
            'explainability_output': 'Oscillator position relative to overbought/oversold levels and trend direction',
