"""Unit tests for the quality_a strategy package.

(a) imports all 8 strategies
(b) asserts StrategyMetadata flags
(c) for each PASSING strategy, runs run_walkforward-style backtest on REAL
    data and asserts the runtime BacktestVerdict criteria
    (win_rate>=50, sharpe>0.5, pf>1.2, ret>-10)
...
def backtest_verdict(strategy, closes, highs, lows, volumes, product_id="BTC-USD"):
    """Replicate the runtime BacktestVerdict on real data."""
...
PASSING = {
    "FisherTransformStochStrategy": ("btc_real_hourly.csv", "AAPL_daily.csv"),
    "WilliamsPctRStrategy": ("BTC-USD_daily.csv", "MSFT_daily.csv"),
    "CciShortReversalStrategy": ("btc_real_hourly.csv", "AAPL_daily.csv"),
    "SessionOpeningRangeBreakoutStrategy": ("BTC-USD_daily.csv", "MSFT_daily.csv"),
    "ChaikinVolatilityBreakoutStrategy": ("BTC-USD_daily.csv", "AAPL_daily.csv"),
}
...
def test_passing_strategy_gate(sid, files):
    ...
    s = cls()
    for fname in files:
        closes, highs, lows, volumes = _load(os.path.join("historical", fname) if "daily" in fname else fname)
        v = backtest_verdict(s, closes, highs, lows, volumes)
        assert v["passed"], f"{sid} failed on {fname}: {v}"
