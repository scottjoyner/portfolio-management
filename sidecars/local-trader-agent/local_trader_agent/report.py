...
def _summary_table(summary: BacktestSummary) -> str:
    rows = [
        ...
        ("Profit Factor", "∞" if summary.profit_factor == float("inf") else f"{summary.profit_factor:,.2f}"),
...
def _trades_table(trades: list[Trade]) -> str:
    ...
    return df.to_html(index=False, classes="trades", border=0, escape=True)
...
def build_figure(data: pd.DataFrame, trades: list[Trade], cfg: BacktestConfig) -> go.Figure:
    fig = make_subplots(
        ...
        subplot_titles=(f"{cfg.ticker} Candles + Trades", "RSI", "Equity Curve"),
    )
    ...
    if trades:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker_size=12,
                name="Buy",
                text=[f"Entry: {p:.2f}" for p in entries["entry_price"]],
            ),
            row=1,
            col=1,
        )
        fig.add_trace(
            go.Scatter(
                ...
                marker_size=12,
                name="Sell",
                text=[f"Exit: {p:.2f}" for p in entries["exit_price"]],
            ),
            row=1,
            col=1,
        )
...
def write_html_report(
    ...
    fig = build_figure(data, trades, cfg)
...
<head>
  ...
  <style>
    body {{ font-family: system-ui, -apple-system, Segoe UI, Roboto, sans-serif; margin: 0; background: #f7f7f8; color: #111827; }}
    header {{ padding: 28px 36px; background: #111827; color: white; }}
    main {{ padding: 24px 36px 48px; max-width: 1280px; margin: auto; }}
    .card {{ background: white; border-radius: 14px; padding: 20px; margin: 18px 0; box-shadow: 0 1px 8px rgba(0,0,0,.08); }}
    table {{ border-collapse: collapse; width: 100%; }}
    th, td {{ padding: 10px 12px; border-bottom: 1px solid #e5e7eb; text-align: left; }}
    th {{ width: 220px; color: #374151; }}
    .trades th {{ width: auto; }}
    code, pre {{ background: #f3f4f6; border-radius: 8px; }}
    pre {{ padding: 14px; overflow-x: auto; }}
    .disclaimer {{ color: #6b7280; font-size: 0.95rem; }}
  </style>
</head>
<body>
  <header>
    <h1>{html.escape(cfg.ticker)} RSI Strategy Backtest</h1>
    <p>Buy when RSI crosses above {cfg.buy_rsi_cross}; sell at +{cfg.take_profit_pct:.2%} take profit or -{cfg.stop_loss_pct:.2%} stop loss; no overlapping positions.</p>
  </header>
  <main>
    <section class=\"card\">
      <h2>Summary</h2>
      <table>{_summary_table(summary)}</table>
    </section>
    <section class=\"card\">
      <h2>Charts</h2>
      {chart_html}
    ...
    <section class=\"card disclaimer\">
      <strong>Disclaimer:</strong> This is research software, not financial advice. Backtests do not guarantee future performance. Validate data quality, fees, slippage, survivorship bias, tax impact, and execution assumptions before making decisions.
    </section>
  </main>
</body>
</html>
