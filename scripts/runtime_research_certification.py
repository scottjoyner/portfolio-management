"""Executable runtime binding for research-tournament-certified strategies.

This module has no broker or order authority.  It converts promotion evidence
that has already passed ChallengerRegistry's full alpha/replay/terminal checks
into a narrow runtime credential only when production can execute the exact
strategy/config/symbol/granularity that was certified.

The first supported executable pilot is configured ``rsi_revert`` through the
same compiled Rust entry point used by canonical replay.  Unsupported strategies
or malformed/incomplete evidence return no executable certification rather than
silently falling back to default parameters.
"""
...
def build_runtime_research_certification(
    ...
    """Mint an executable credential only for an exactly reproducible runtime.

    ChallengerRegistry must verify alpha evidence, replay provenance, terminal
    evidence, and lineage before calling this function.  This function then
    performs the additional *executability* checks that distinguish a research
    promotion from a runtime-certified strategy.
    """

    if not isinstance(challenger, dict) or not isinstance(alpha_evidence, dict) or not isinstance(terminal_evidence, dict):
        return None
    attestation = alpha_evidence.get("replay_attestation")
    terminal_dataset = terminal_evidence.get("terminal_dataset")
    terminal_metrics = terminal_evidence.get("metrics")
    parameters = challenger.get("parameters")
    strategy_name = terminal_evidence.get("strategy_name")
    if not isinstance(attestation, dict) or not isinstance(terminal_dataset, dict) or not isinstance(terminal_metrics, dict):
        return None

    # Pilot scope is intentionally narrow.  Do not call a strategy executable
    # merely because it passed research if production cannot reproduce it.
    if strategy_name != "rsi_revert" or attestation.get("strategy_name") != strategy_name:
        return None
    try:
    ...
    if not normalized_config:
        return None
    if normalized_config != alpha_evidence.get("candidate_config"):
        return None
    if normalized_config != attestation.get("strategy_config"):
        return None
    if normalized_config != terminal_evidence.get("candidate_config"):
        return None
    if normalized_config != terminal_evidence.get("strategy_config"):
        return None

    config_hash = stable_hash(normalized_config)
    if config_hash != alpha_evidence.get("candidate_config_hash"):
        return None
    if config_hash != terminal_evidence.get("candidate_config_hash"):
        return None
    if config_hash != terminal_evidence.get("strategy_config_hash"):
        return None

    candidate_id = challenger.get("id")
    if candidate_id != alpha_evidence.get("candidate_id") or candidate_id != terminal_evidence.get("candidate_id"):
        return None
    candidate_source_sha = alpha_evidence.get("candidate_source_sha")
    if candidate_source_sha != terminal_evidence.get("candidate_source_sha"):
        return None
    alpha_hash = alpha_evidence.get("evidence_hash")
    if alpha_hash != terminal_evidence.get("alpha_validation_evidence_hash"):
        return None
    ...
    metrics = copy.deepcopy(terminal_metrics)
    core = {
        ...
        "experiment_id": required_strings["experiment_id"],
        ...
        "terminal_metrics_hash": stable_hash(metrics),
        "replay_attestation_hash": stable_hash(attestation),
        "evaluation_lineage_id": required_strings["evaluation_lineage_id"],
    }
    return {
        **core,
...
def load_active_research_certification(path: Path | str = ACTIVE_CONFIG_PATH) -> dict[str, Any] | None:
    ...
    if not isinstance(payload, dict) or payload.get("schema_version") != 2:
        return None
    certification = payload.get("research_certification")
    ...
    if payload.get("terminal_holdout_evidence_hash") != certification.get("terminal_holdout_evidence_hash"):
        return None
    if payload.get("runtime_certification_hash") != certification.get("certification_hash"):
        return None
...
def run_certified_current_signal(
    ...
    *,
    ...
    No Python/default fallback is permitted.  If the compiled configured Rust
    path is unavailable, the runtime yields no certified signal.
    """
