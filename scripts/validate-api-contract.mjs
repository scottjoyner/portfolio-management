...
for (const path of [contractPath, serverPath, routerPath]) {
  if (!existsSync(path)) {
...
for (const route of requiredRoutes) {
  if (!contract.includes(route)) {
    console.error(`api contract validation failed: contract missing ${route}`);
    process.exit(1);
  }
...
const requiredPatterns = [
  ...
  '/api/backtests/:id/report',
  '/api/approvals/:id/decision',
  '/api/paper-executions/:id/stop'
];

for (const route of requiredPatterns) {
  if (!contract.includes(route)) {
    console.error(`api contract validation failed: contract missing ${route}`);
    process.exit(1);
  }
