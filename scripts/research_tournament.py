#!/usr/bin/env python3
"""One-shot research experiment lineage and untouched terminal-holdout evidence.

This module deliberately contains no broker/order-routing authority.  It closes a
research-trust gap: repeated challenger search must not repeatedly optimize on
the final test window.

The trusted workflow is:

    canonical full dataset
      -> immutable search/embargo/terminal split commitment
      -> many replay-bound candidate trials on search data only
      -> deterministic pre-holdout selection
      -> exactly one selected candidate opens/replays terminal holdout
      -> hash-bound terminal evidence + append-only lineage

The terminal holdout can still be reverified later for integrity.  "One-shot"
means an experiment may create only one terminal evaluation event; source
reverification of that same committed result is not a new candidate trial.
"""
...
SELECTION_POLICY_VERSION = "validated_net_pnl_then_quality_v1"
TERMINAL_METHOD = "canonical_one_shot_terminal_holdout_v1"
...
def build_experiment_plan_from_snapshot(
    ...
    *,
    ...
    """Commit an immutable chronological search/embargo/terminal partition.

    The returned plan contains only manifests/hashes for the terminal window,
    never its OHLCV rows.  Trusted infrastructure necessarily sees the source
    rows while making the commitment; research agents should receive the plan,
...
def _terminal_result_core(evidence: dict[str, Any]) -> dict[str, Any]:
    fields = (
        ...
        "alpha_validation_evidence_hash", "terminal_dataset", "strategy_name",
...
def verify_terminal_holdout_evidence(
    ...
    *,
    ...
    """Verify terminal evidence, source replay, and optional one-shot lineage."""
    ...
    required = {
        ...
        "selection_lineage_id", "selection_lineage_event_hash",
        "terminal_lineage_id", "terminal_lineage_event_hash", "evidence_hash",
    }
    reasons: list[str] = []
    ...
    try:
        ...
        supplied_evidence_hash = final_core.pop("evidence_hash", None)
        if stable_hash(final_core) != supplied_evidence_hash:
            reasons.append("terminal_evidence_hash_mismatch")
        result_core = _terminal_result_core(evidence)
        if stable_hash(result_core) != evidence.get("terminal_result_hash"):
        ...
        normalized_config = normalize_strategy_config(evidence["strategy_name"], evidence["strategy_config"])
        if normalized_config != evidence["strategy_config"]:
            reasons.append("terminal_strategy_config_not_normalized")
        if stable_hash(normalized_config) != evidence["strategy_config_hash"]:
        ...
        recomputed_metrics, recomputed_passed, recomputed_reasons = _derive_terminal_result(
            evidence["returns"], periods_per_year=periods_per_year, policy=policy
        )
        if recomputed_metrics != evidence["metrics"]:
            reasons.append("terminal_metrics_mismatch")
        if recomputed_passed is not evidence["passed"]:
            reasons.append("terminal_pass_state_mismatch")
        if recomputed_reasons != evidence["reasons"]:
        ...
        try:
            ...
            if snapshot["manifest"] != dataset:
                ...
                regenerated = replay_trade_returns_rust(
                    ...
                    snapshot["rows"],
                    warmup=int(evidence["warmup"]),
                    fee_bps=float(evidence["fee_bps"]),
...
class ResearchTournament:
    """Durable state machine for search-only trials and one terminal evaluation."""
    ...
    def create_from_snapshot(
        ...
        *,
        ...
        record = {
            ...
            "experiment_lineage_event_hash": event["event_hash"],
            "trials": [],
            "selection": None,
            "terminal_holdout_evidence": None,
            "created_at": _utc_now(),
        }
    ...
    def register_candidate(
        ...
        *,
        ...
        event = self.lineage.append(
            ...
            actor=actor,
            parents=[experiment["experiment_lineage_id"]],
        )
    ...
    def seal_selection(
        ...
        *,
        ...
        event = self.lineage.append(
            ...
            actor=actor,
            parents=[row["lineage_id"] for row in experiment["trials"]],
        )
    ...
    def run_terminal_holdout(
        ...
        experiment_id: str,
        *,
        policy: TerminalHoldoutPolicy | None = None,
        ...
        if existing_events or experiment.get("terminal_holdout_evidence") is not None:
        ...
        snapshot = load_canonical_snapshot(
            kind=terminal_dataset["kind"],
            symbol=terminal_dataset["symbol"],
            granularity=int(terminal_dataset["granularity"]),
            start_ts=float(terminal_dataset["start_ts"]),
            end_ts=float(terminal_dataset["end_ts"]),
        )
        ...
        returns = replay_trade_returns_rust(
            ...
            max_hold_bars=int(replay.get("max_hold_bars", 0)),
        ...
        terminal_result_hash = stable_hash(core)
        event = self.lineage.append(
            "terminal_holdout",
            {
                "experiment_id": experiment_id,
        ...
        evidence = {
            **core,
            ...
            "terminal_lineage_id": event["id"],
            "terminal_lineage_event_hash": event["event_hash"],
        }
        evidence["evidence_hash"] = stable_hash(evidence)
        valid, verify_reasons = verify_terminal_holdout_evidence(
            evidence,
            lineage=self.lineage,
            reverify_source=True,
        )
        if not valid:
            raise ValueError("terminal holdout evidence failed self-verification: " + ",".join(verify_reasons))
