...
export const RESEARCH_CERTIFICATION_TYPE = 'research_tournament_terminal_runtime_v1';
...
export function verifyResearchCertification(certification, { strategyId = null, symbol = null } = {}) {
  ...
  if (!certification.candidate_config || typeof certification.candidate_config !== 'object' || Array.isArray(certification.candidate_config)) {
    reasons.push('research_certification_config_required');
  } else {
    try {
      if (researchStableHash(certification.candidate_config) !== certification.candidate_config_hash) {
        reasons.push('research_certification_config_hash_mismatch');
      }
    } catch {
      reasons.push('research_certification_config_not_canonicalizable');
    }
  }

  if (!certification.terminal_metrics || typeof certification.terminal_metrics !== 'object' || Array.isArray(certification.terminal_metrics)) {
    reasons.push('research_certification_terminal_metrics_required');
  } else {
    try {
      if (researchStableHash(certification.terminal_metrics) !== certification.terminal_metrics_hash) {
        reasons.push('research_certification_terminal_metrics_hash_mismatch');
      }
    } catch {
      reasons.push('research_certification_terminal_metrics_not_canonicalizable');
    }
  }
