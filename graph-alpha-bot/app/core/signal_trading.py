...
from app.utils.logger import TradeEventLogger
...
class NewsSentimentAnalyzer:
        ...
        try:
            ...
            for symbol in self.symbols:
                ...
                if len(matching) == 0:
                    ...
                    avg_sentiment = sum(
                        a.get("sentiment_score", 0.5) for a in matching
                    ) / len(matching)
    ...
    def get_signal_reason(self, symbol: str, sentiment: float, news_count: int) -> str:
        ...
        return (
            f"{direction} news sentiment ({sentiment:.2f}) from {news_count} articles"
        )
...
class SignalGenerator:
    """Unified signal generator combining news sentiment and BTC-XXX volatility strategies."""
    ...
    def __init__(self, config: Optional[SignalConfig] = None):
        ...
        self._event_logger = TradeEventLogger()

        # Initialize unified signal generator for enhanced strategy integration
        try:
            from app.strategies.unified_signal_generator import UnifiedSignalGenerator, UnifiedSignalConfig
            
            unified_config = UnifiedSignalConfig(
                symbols=self.config.symbols,
                sentiment_threshold=0.25,
                enable_strategy_signals=True,
                enable_news_signals=True
            )
            self.unified_generator = UnifiedSignalGenerator(unified_config)
            self.use_unified = True
            self._logger.info("Unified signal generator initialized with BTC-XXX volatility strategies")
        except Exception as e:
            self._logger.warning(f"Failed to initialize unified signal generator: {e}")
    ...
    def generate_signals(self) -> List[TradingSignal]:
        ...
        self._logger.info("GENERATING UNIFIED SIGNALS")
        self._logger.info("=" * 60)

        # Step 1: Get news sentiment scores
        sentiment_data = self.analyzer.analyze_sentiment()
        logger.info(
            f"News sentiment analysis complete: {len(sentiment_data)} symbols covered"
        )
        ...
        for symbol, (score, change) in tech_data.items():
            ...
            logger.info(
                f"  {symbol}: technical={score:.3f}, price_change={change}% ({direction_str})"
            )

        # Step 3: Generate unified signals using unified signal generator if available
        new_signals = []
        
        if self.use_unified:
            self._logger.info("Using unified signal generator with BTC-XXX volatility strategies")
            try:
                # Generate signals using unified system
                unified_signals = self.unified_generator.generate_signals()
                
                # Convert unified signals to TradingSignal format
                for unified_sig in unified_signals:
                    signal = TradingSignal(
                        symbol=unified_sig.symbol,
                        direction=unified_sig.direction,
                        confidence=unified_sig.confidence,
                        sentiment_score=unified_sig.sentiment_score,
                        technical_score=unified_sig.technical_score,
                        price_change_pct=unified_sig.price_change_pct,
                        news_count=unified_sig.news_count,
                        signal_reason=f"{unified_sig.strategy_name}: {unified_sig.signal_reason}",
                        timestamp=unified_sig.timestamp,
                    )
                    new_signals.append(signal)
                    
                    self._logger.info(
                        f"✓ {signal.symbol} {signal.direction}: conf={signal.confidence:.2f}, "
                        f"strategy={signal.signal_reason}, "
                        f"sentiment={signal.sentiment_score:.3f}, tech={signal.technical_score:.3f}"
                    )
                    
            except Exception as e:
                self._logger.error(f"Unified signal generation failed: {e}")
                self.use_unified = False

        # Step 4: Fallback to traditional signal generation if unified fails
        if not self.use_unified:
            self._logger.info("Falling back to traditional signal generation")
            
            # Step 4: Generate unified signals using traditional approach
            for symbol in self.config.symbols:
                ...
                sentiment_contrib = (
                    abs(sentiment_score) * 0.4 if sentiment_score != 0 else 0
                )
                technical_contrib = (
                    abs(technical_score) * 0.35 if technical_score != 0 else 0
                )
                ...
                if (
                    sentiment_score > self.config.sentiment_threshold
                    and technical_score > 0
                ):
                    direction = "LONG"
                elif (
                    abs(sentiment_score) < self.config.sentiment_threshold
                    and abs(technical_score) < 0.1
                ):
                    logger.info(
                        f"{symbol}: No strong signal - skipping (neutral across all sources)"
                    )
                    continue
                ...
                if direction:
                    ...
                    signal_reason = (
                        ", ".join(reason_parts)
                        if reason_parts
                        else f"Sentiment-driven {direction.lower()}"
                    )
                    ...
                    logger.info(
                        f"✓ {symbol} {direction}: conf={confidence:.2f}, "
...
class PaperTradingExecutor:
    ...
    def get_order_size(self, symbol: str) -> float:
        ...
        total_value = self.portfolio["USD"] + sum(
            self.prices.get(f"{t}-USD", 0) * qty
            for t, qty in self.portfolio.items()
            if t != "USD"
        )
        max_position = total_value * 0.10

        current_value = self.prices.get(f"{symbol}-USD", 0) * self.positions.get(
            symbol.split("-")[0], 0
        )
    ...
    def execute_signal(self, signal: TradingSignal) -> Optional[dict]:
        ...
        logger.info(
            f"    Sentiment: {signal.sentiment_score:.3f}, News articles: {signal.news_count}"
        )

        # Log signal to structured event logger
        self._event_logger.log_signal(signal.to_dict())

        # Check cooldown
        if signal.symbol in self.last_signal_times:
            minutes_since_last = (
                datetime.now() - self.last_signal_times[signal.symbol]
            ).total_seconds() / 60
        ...
        if signal.direction == "LONG":
            ...
            price = (
                ...
                else self.prices.get(f"{signal.symbol}-USD", 100.0)
            )
    ...
    def display_portfolio(self):
        ...
        logger.info(
            f"BTC: {self.portfolio.get('BTC', 0):.4f} @ ${self.prices['BTC-USD']:,.2f} = ${self.portfolio.get('BTC', 0) * self.prices['BTC-USD']:,.2f}"
        )
        logger.info(
            f"ETH: {self.portfolio.get('ETH', 0)} @ ${self.prices['ETH-USD']:,.2f} = ${self.portfolio.get('ETH', 0) * self.prices['ETH-USD']:,.2f}"
        )
