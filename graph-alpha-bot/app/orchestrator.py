...
class PipelineOrchestrator:
    ...
    def __init__(self):
        ...
        # Structured loggers
        self.trade_logger = TradeEventLogger()
        self.metrics_logger = PipelineMetricsLogger()
    ...
    def run_signal_generation(self) -> List[dict]:
        ...
        for symbol in self.config.news_symbols:
            ...
            if kg_file.exists():
                try:
                    ...
                    for article in kg.get("articles", []):
                        ...
                        if pub_time >= cutoff and symbol.replace(
                            "-USD", ""
                        ) in article.get("tickers", []):
            ...
            if symbol in self.last_signal_times:
                if datetime.utcnow() - self.last_signal_times[symbol] < timedelta(
                    seconds=self.config.signal_cooldown_seconds
                ):
                    continue
            ...
            if (
                news_count >= 2
                and avg_sentiment >= self.config.sentiment_threshold_long
            ):
                signal = {
                    "symbol": symbol,
                    "direction": "LONG",
            ...
            # Log each generated signal for audit trail
            if (
                news_count >= 2
                and avg_sentiment >= self.config.sentiment_threshold_long
            ):
            ...
            # Log signal/opportunity detection metrics
            self.metrics_logger.log_opportunity(
                {
                    "symbol": symbol,
                    "type": "sentiment_long",
                    "confidence": round(avg_sentiment, 3),
                    "reason": f"News count: {news_count}, avg sentiment: {avg_sentiment:.2f}",
                }
            )
    ...
    def execute_signal(self, signal: dict) -> dict:
        ...
        if self.circuit_open:
            if (datetime.utcnow() - self.circuit_opened_at) < timedelta(seconds=600):
                ...
                self.metrics_logger.log_circuit_breaker("closed")
        ...
        try:
            ...
            # Log circuit breaker state change
            self.metrics_logger.log_circuit_breaker("open")

            # Log error with context
            self.error_logger.log_error(e, "execute_signal", "orchestrator")
...
def main():
    ...
    while orchestrator.running:
        try:
            ...
            # Log pipeline cycle metrics
            orchestrator.metrics_logger.log_cycle(
                cycle_num=cycle_num,
