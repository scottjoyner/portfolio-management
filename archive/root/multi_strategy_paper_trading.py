...
import sys, asyncio, json, time, datetime, math, logging
    ...
    def _seed_history(self, symbols: List[str]) -> None:
        if self.history_client is None:
            ...
            try:
                ...
                logger.info("Initialized CBClient for history seeding")
            except Exception as e:
                logger.warning("CBClient init failed for history seeding: %s", e)
                self.history_client = None
                for sym in symbols:
                    self.market_history[sym] = []
                print(" WARN: No historical data available — strategies will see empty history, expect 0 signals")
                return
        try:
            from coinbase.src.data import fetch_candles_df
        except Exception as e:
            logger.warning("fetch_candles_df import failed: %s", e)
            for sym in symbols:
                self.market_history[sym] = []
            print(" WARN: No historical data available — strategies will see empty history, expect 0 signals")
            return
        ...
        for sym in symbols:
            try:
                ...
                if self.market_history[sym]:
                    loaded += 1
                else:
                    self.market_history[sym] = []
            except Exception as e:
                logger.warning("Failed to seed history for %s: %s", sym, e)
                self.market_history[sym] = []
        print(f" History seeded: {loaded}/{len(symbols)} symbols have data")
        if loaded == 0:
            print(" WARN: Zero historical data loaded — strategies will produce NO signals")
        logger.info("History seeded: %d/%d symbols", loaded, len(symbols))
            ...
            try:
                ...
                for sig, pdata in all_raw_signals:
                    try:
                        ...
                        logger.warning("Signal scoring failed for %s: %s", sig.symbol if hasattr(sig, 'symbol') else '?', e)
                        print(f"   WARN: Signal scoring error for {getattr(sig, 'symbol', '?')}: {e}")
                        continue
